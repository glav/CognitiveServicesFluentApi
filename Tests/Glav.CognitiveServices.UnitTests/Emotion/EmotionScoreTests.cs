...
namespace Glav.CognitiveServices.UnitTests.Emotion
{
    public class EmotionScoreTests
    {
        ...
        public async Task ShouldCalculateCorrectScoreFromDefaultSet()
        {
            ...
            Assert.Equal(result.ImageRecognitionAnalysis.ScoringEngine.EvaluateScore(actualItem.scores.anger).Name, 
                EmotionRangeScoreLevels.DefinitelyNegative);
            Assert.Equal(result.ImageRecognitionAnalysis.ScoringEngine.EvaluateScore(actualItem.scores.contempt).Name, 
                EmotionRangeScoreLevels.DefinitelyNegative);
            Assert.Equal(result.ImageRecognitionAnalysis.ScoringEngine.EvaluateScore(actualItem.scores.disgust).Name, 
                EmotionRangeScoreLevels.DefinitelyNegative);
            Assert.Equal(result.ImageRecognitionAnalysis.ScoringEngine.EvaluateScore(actualItem.scores.fear).Name, 
                EmotionRangeScoreLevels.DefinitelyNegative);
            Assert.Equal(result.ImageRecognitionAnalysis.ScoringEngine.EvaluateScore(actualItem.scores.happiness).Name, 
                EmotionRangeScoreLevels.DefinitelyPositive);
            Assert.Equal(result.ImageRecognitionAnalysis.ScoringEngine.EvaluateScore(actualItem.scores.sadness).Name,
                EmotionRangeScoreLevels.DefinitelyNegative);
            Assert.Equal(result.ImageRecognitionAnalysis.ScoringEngine.EvaluateScore(actualItem.scores.surprise).Name, 
                EmotionRangeScoreLevels.DefinitelyNegative);
        }
